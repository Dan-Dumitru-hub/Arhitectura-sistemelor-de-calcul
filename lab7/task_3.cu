...
__global__ void modify( int *a,  int *b, int *c, int N) {
...
int main(void) {
   ...
   modify<<<blocks_no, 256>>>(device_array_a, device_array_b,device_array_c, n);



    // TODO 5: Copy from device to host
      cudaMemcpy(host_array_a, device_array_a, BUF_32M, cudaMemcpyDeviceToHost);
      cudaMemcpy(host_array_b, device_array_b, BUF_32M, cudaMemcpyDeviceToHost);
